...
WebServ::WebServ(std::string configFile) {
    ...
    this->reserved_sockets = this->max_fds = this->open_fds = this->getNumberOfSockets();
};
...
bool WebServ::checkTriggeredFd(int fd) {
    for (size_t i = 0; i < reserved_sockets; i += 1) {
        if (fd == this->servers.at(i).getSocketFd())
            return true;
    }
    return false;
}
...
void WebServ::runServers(void) {
    ...
    while (!g_stopServer) {
        num = open_fds;
        if (poll(pollfds, num, 2000) == -1 && !g_stopServer)
        ...
        for (size_t i = 0; i < open_fds; i += 1) {
            ...
            if (pollfds[i].revents & POLLIN) {
                if (this->checkTriggeredFd(pollfds[i].fd)) {
                    if (open_fds == max_fds) {
                        pollfds = (struct pollfd *)realloc(pollfds, sizeof(struct pollfd) * (open_fds * 2));
                        max_fds *= 2;
                    }
            ...
            if (pollfds[i].revents & POLLOUT) {
                if (!clients.at(i - reserved_sockets).preparedToSend())
                    continue;
                clients.at(i - reserved_sockets).setTargetServer(getServerByName(clients.at(i - reserved_sockets).getRequest(), clients.at(i - reserved_sockets).getTargetServer()));
